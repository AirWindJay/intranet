...
@section('content')
    ...
    @foreach ($mydata as $me)
        ...
        @endif
        @if($me->role_id == 12)
            <a href="/nurse2/appendnurse/{{$currentward->id}}"><button class="btn btn-success">Add Nurse</button></a>
            <a href="/nurse2/nurseschedule/edit/{{$currentward->id}}"><button class="btn btn-success">Edit Schedule </button></a>
        @endif
    @endforeach
        <div class="ui-select">
            <div class="table-responsive" style="overflow-x:auto;">
                <h3 id="currentmonth"></h3>
                @include('nursing.nurselevel2.table')
                <hr>
                <h3>Next Month</h3>
                @include('nursing.nurselevel2.table2')
            </div>
        </div>
@endsection